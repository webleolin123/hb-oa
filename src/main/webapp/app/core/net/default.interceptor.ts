...
export class DefaultInterceptor implements HttpInterceptor {
    ...
    private checkStatus(ev: HttpErrorResponse) {
        ...
        const errorDetail=ev.error?ev.error.detail:ev.message;
        this.injector.get(NzNotificationService).error(
            `请求错误 ${ev.status}:${errorDetail}`,
            errortext
        );
    }
    ...
    private handleData(ev: HttpErrorResponse): Observable<any> {
        ...
        if (ev.status > 0) {
            ...
            if(ev.status==200){
                this.nzCustomAlertService.success(ev.statusText);
            }
        }
        this.checkStatus(ev);
        // 业务处理：一些通用操作
        switch (ev.status) {
            ...
            default:
                if (ev instanceof HttpErrorResponse) {
                    ...
                    this.nzCustomAlertService.error(ev.message);
                }
                break;
        }
        return of(ev);
