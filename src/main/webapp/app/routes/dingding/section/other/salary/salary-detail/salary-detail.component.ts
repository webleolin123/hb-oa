...
import {NzCustomAlertService,SALARY_KEYCODE_RECORD} from "@shared";
...
export class SalaryDetailComponent implements OnInit, OnDestroy {
    ...
    sendHttp() {
        // 获取当前请求页面所有数据
        this.salaryDetailService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            keyCode: this.cache.getNone(SALARY_KEYCODE_RECORD),
            sort: this.sortArr,
        }).subscribe(
    ...
    search() {
        ...
        if (this.searchContent == '') {
            this.salaryDetailService.search({
                keyCode: this.cache.getNone(SALARY_KEYCODE_RECORD),
