...
@Component({
    selector: 'load-upload-detail',
    templateUrl: './load-upload-detail.component.html',
})
export class LoadUploadDetailComponent implements OnInit {
    parentData: any;

    shelve: LoadUploadModel;
    ...
    constructor(
        private shelveService: LoadUploadService,
    ...
    load(){
        if (this.parentData.id) {
            this.shelveService.find(this.parentData.id).subscribe(
    ...
    private onSuccess(data) {
        ...
        if (this.shelve.approveInfoDTO.firstApprovers.length != 0) {
            this.user=[];
            this.shelve.approveInfoDTO.firstApprovers.forEach((myCopyTo) => {
                if (myCopyTo != null) {
        ...
        if (this.shelve.approveInfoDTO.firstApprovers.length != 0) {
            this.user=[];
            this.shelve.approveInfoDTO.firstApprovers.forEach((myApprover) => {
                if (myApprover != null) {
