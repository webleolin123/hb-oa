...
//数据初始化，排序
    ...
    add(){
        this.modal
            .static(PriceAddComponent, {parentData: {id: null}})
            .subscribe(() => {//回调函数 重新请求新数据
                this.loadAll();
            });
    }
    edit(id) {
        this.modal
            .static(PriceAddComponent, {parentData: {id: id}})
            .subscribe(() => {//回调函数 重新请求新数据
                this.loadAll();
            });
    }
    ...
    check(id) {
        this.modal
            .static(PriceDetailComponent, {parentData: {id: id}})
            .subscribe(() => {//回调函数 重新请求新数据
                this.loadAll();
        });
    }
    delete(price) {
        let html = `
                店铺:<span>${price.approveInfoDTO.shopName}</span><br>
                品牌:<span>${price.approveInfoDTO.brandName}</span><br>
                商品:<span>${price.approveInfoDTO.goodName}</span><br>
        `;
        this.nzCustomAlertService.confirm('确定删除该条记录?', html, () => {
            if(this.numberOfElements==1){
                this.page = 1;
            }
            this.priceService.delete(price.id).subscribe((response) => {
